//! Benchmark: Metal GPU Fp6 arithmetic vs arkworks CPU
//!
//! Run with: cargo bench --bench metal_fp6 --features metal-gpu,arkworks
...
fn ark_to_gpu(f: &ArkFq6) -> Fp6Limbs {
    Fp6Limbs {
...
fn bench_fp6_mul(c: &mut Criterion) {
    let mut gpu = MetalGpu::new();
    let pipeline = gpu.pipeline("fp6_mul_test").clone();
    let mut group = c.benchmark_group("fp6_mul");

    for &n in SIZES {
        let mut rng = ark_std::test_rng();
        let a_ark: Vec<ArkFq6> = (0..n).map(|_| ArkFq6::rand(&mut rng)).collect();
        let b_ark: Vec<ArkFq6> = (0..n).map(|_| ArkFq6::rand(&mut rng)).collect();
        let a_gpu: Vec<Fp6Limbs> = a_ark.iter().map(ark_to_gpu).collect();
        let b_gpu: Vec<Fp6Limbs> = b_ark.iter().map(ark_to_gpu).collect();
...
fn bench_fp6_sqr(c: &mut Criterion) {
    let mut gpu = MetalGpu::new();
    let pipeline = gpu.pipeline("fp6_sqr_test").clone();
    let mut group = c.benchmark_group("fp6_sqr");
...
criterion_group!(benches, bench_fp6_mul, bench_fp6_sqr);
