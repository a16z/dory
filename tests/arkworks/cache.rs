use dory_pcs::backends::arkworks::{ArkG1, ArkG2, ArkGT, ArkGTCompressed, BN254};
...
#[test]
fn multi_pair_compressed_empty() {
    let empty_g1: Vec<ArkG1> = vec![];
    let empty_g2: Vec<ArkG2> = vec![];

    let result = BN254::multi_pair_compressed(&empty_g1, &empty_g2);
    assert_eq!(ArkGT::from(result), ArkGT::from(ArkGTCompressed::default()));
}
...
#[test]
#[should_panic(expected = "multi_pair_compressed requires equal length vectors")]
fn multi_pair_compressed_length_mismatch() {
    let mut rng = thread_rng();

    let ps: Vec<ArkG1> = (0..5).map(|_| ArkG1::random(&mut rng)).collect();
    let qs: Vec<ArkG2> = (0..3).map(|_| ArkG2::random(&mut rng)).collect();

    BN254::multi_pair_compressed(&ps, &qs);
}
...
#[cfg(feature = "cache")]
#[test]
fn multi_pair_compressed_with_cache_optimization() {
    let mut rng = thread_rng();
    let n = 20;
    ...
    let result = BN254::multi_pair_compressed(ps, qs);
    ...
    assert_eq!(ArkGT::from(result), expected);
}
